def count_letters(text): # TODO  Напишите функцию count_letters
    ...
    for char in text:
        if char.isalpha():
            char = char.lower()
            if char in letter_counts: # Увеличиваем счетчик для данной буквы
...
frequency_dict = calculate_frequency(letter_counts) # Вычисление частоты букв

for letter, frequency in frequency_dict.items():
    print(f"{letter}: {frequency:.2f}") # Вывод результата в столбик